#Week 2 Thursday/Friday Application

'''
Requirements

Requires the addition of vendors contact details such as 
Name or company name, phone number, and email address

Requires the ability to search specific vendors

Requires that you are able to delete a vendor

Requires ability to update vendors details

Requires ability to add extra details to vendors contactdt

Add a simple terminal menu that allows you to select
which operation you would like to carry out
'''

operation_selection = ["A", "S", "D", "U", "E"]
vendors_list = []

while True:
    operation = input("\nTo choose an operation, type:\nA to add a vendor's contact details,\nS to search current vendors,\nD to delete from the list of vendors,\nU to update from list of current vendors, or\nE to add extra details to current vendors:")
